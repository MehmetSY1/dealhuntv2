        ...
        vm.deals.observe(viewLifecycleOwner) { state ->
            ...
            when (state) {
                is UiState.Success -> {
                    ...
                    aiAdapter.submitList(state.data.take(8))
                }
                is UiState.Error -> {
                    // Sessizce göster, swipe ile yenilenebilir
                }
                else -> {}
            }
        }
...
class ExploreFragment : Fragment() {
    ...
    private val genres = listOf("🔥 Trend","⚔️ RPG","🎯 Aksiyon","🧩 Strateji","👾 Indie","🏎️ Yarış","⚽ Spor","🔫 FPS","🌍 Açık Dünya","🧟 Korku")
    ...
    override fun onViewCreated(v: View, s: Bundle?) {
        ...
        vm.search.observe(viewLifecycleOwner) { state ->
            when (state) {
                ...
                is UiState.Error   -> { b.progressExplore.visibility = View.GONE }
                else -> {}
            }
        }
        selectGenre(0)
...
class WishlistFragment : Fragment() {
    ...
    override fun onViewCreated(v: View, s: Bundle?) {
        ...
        if (list.isEmpty()) {
            ...
            b.rvWishlist.visibility = View.GONE
        } else {
            b.emptyView.visibility = View.GONE
            b.rvWishlist.visibility = View.VISIBLE
...
class AiFragment : Fragment() {
    ...
    override fun onViewCreated(v: View, s: Bundle?) {
        ...
        vm.rate.observe(viewLifecycleOwner) { rate -> adapter.tlRate = rate; adapter.notifyDataSetChanged() }
        vm.deals.observe(viewLifecycleOwner) { state ->
            if (state is UiState.Success) adapter.submitList(state.data.shuffled())
        }
        vm.loadDeals()
    }
