...
class PlatformPriceAdapter(private val onBuy: (PlatformPrice) -> Unit) :
    ...
    inner class VH(val b: ItemPlatformPriceBinding) : RecyclerView.ViewHolder(b.root) {
        fun bind(p: PlatformPrice) {
            ...
            b.tvCurrentPrice.setTextColor(ctx.getColor(if (p.isBestDeal) R.color.accent_green else R.color.text_primary))
            b.root.setBackgroundResource(if (p.isBestDeal) R.drawable.bg_best_deal_card else R.drawable.bg_platform_card)
