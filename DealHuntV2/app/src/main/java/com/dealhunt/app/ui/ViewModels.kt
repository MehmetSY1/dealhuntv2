...
class MainViewModel : ViewModel() {
    ...
    fun searchGames(query: String) {
        ...
        searchJob = viewModelScope.launch {
            ...
            } catch (e: Exception) { _search.value = UiState.Error("Bağlantı hatası. İnternet bağlantını kontrol et.") }
        }
    }
    ...
    fun loadDeals() {
        viewModelScope.launch {
            ...
            try {
                // Kuru ve fırsatları paralel çek
                val rateDeferred = async { repo.getCurrentRate() }
                val dealsDeferred = async { repo.getFeaturedDeals() }
                _rate.value = rateDeferred.await()
                _deals.value = UiState.Success(dealsDeferred.await())
            } catch (e: Exception) {
                _deals.value = UiState.Error("Fırsatlar yüklenemedi")
            }
        }
    }
